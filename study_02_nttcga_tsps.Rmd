...
Our raw dataset, containing 25360 genes, will be reduced to a geneset containing 117 tissue-specific genes belonging to lung, kidney, liver, prostate tissue. This will allow us to confirm the correlation between tissue samples and expressed genes and verify that clustering is consistent.
...
We proceed to compare both RPM and DESeq2 nmethods against each other in 3 key aspects: Cluster consistency, size factor correlation and raw value transformation.
...
We observe 4 mostly consistent gene clusters throughout the comparison, with the exception of rpmC$deseq2A, where 1 gene is displaced from each cluster. Clustering is somewhat consistent between these methods.
...
In this plot, a separation into two main groups is first noticed. Two groups whose values increase at different rates but are positively correlated. On the other hand, we can also observe a 40-fold factor between RPM and DESeq2 size factors, just as in the LIHC study.
...
Finally, we can note that, when it comes to small raw values, the RPM method has a tendency to crush these values. The crushing effect is corrected by normalizing with RPM40 (RPM × 40). 
