...
We observe mostly consistent gene clusters throughout the comparison (with the exception of rpmC$deseq2A, where 1 gene is displaced from each cluster), which is to be. 
...
In each plot excepting `rpm40_sf` vs `rpm_sf``(because corr = 1) we can observe a phenomena where the ratios of the scaling factors form two distinct groups. ## We can assume this is due to a separation between housekeeping and tissue-specific genes. It is to note that the 40-fold factor is also present when comparing RPM to EdgeR and we can safely assume that an X-fold factor is not due to a difference in the input data but rather due to the normalization method. 
